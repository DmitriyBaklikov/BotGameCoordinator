...
**Response contract**: Every user interaction with the bot — whether a command, callback query (inline keyboard tap), or plain-text message processed by the FSM — **must** produce a visible response sent back to Telegram. The bot never silently consumes an update. Responses take one of three forms:

1. **Text message** — sent via `sendMessage` to the originating chat (e.g., welcome text, error messages, confirmation summaries).
2. **Text message with inline keyboard** — sent via `sendMessage` with an `InlineKeyboardMarkup` reply markup (e.g., FSM step prompts, event cards with voting buttons, paginated listings).
3. **Callback query answer** — sent via `answerCallbackQuery` to acknowledge an inline-keyboard tap, optionally with a short toast notification (e.g., "Vote recorded", "Selection saved"). When a callback also changes displayed content, the bot additionally calls `editMessageText` / `editMessageReplyMarkup` to update the original message in-place.

If processing fails unexpectedly, the bot responds with a generic error message (i18n key `bot.generic_error`) rather than leaving the user without feedback.
...
| Command | Scope | Role | Description | Bot Response |
|---|---|---|---|---|
| `/start` | Private | Any | Register user; show main menu | Welcome text message with inline keyboard (role selection, main menu options) |
| `/newgame` | Private | Organizer | Begin multi-step game creation FSM | First FSM step: sport-type selection inline keyboard. If unauthorized: error message with registration prompt. If at event limit: limit notice with inline buttons to archive existing events. |
| `/mygames` | Private | Organizer | List own active events with management actions | List of active games as text with inline keyboards per game (manage participants, cancel). If no active games: informational message. |
| `/archive` | Private | Organizer | List past/cancelled own events; allow relaunch | Paginated list of archived/cancelled games with Relaunch inline button per entry. If empty: informational message. |
| `/publicgames` | Private/Group | Any | Browse public active events with filters | Paginated list of public active event cards with filter inline keyboard (sport, location, organizer). If no matching games: informational message. |
| `/settings` | Private | Any | Manage notification subscriptions, locale preference | Settings menu as inline keyboard (Language, My subscriptions, Subscribe to organizer, My role). |
...
Steps in order. Each step sends a response back to Telegram — either an inline keyboard or a text prompt — and waits for user input before advancing.

1. **Sport type** — bot responds with inline keyboard: basketball, football, volleyball, hockey, tennis, badminton, other.
2. **Event type** — bot responds with inline keyboard: Game | Training.
3. **Date & Time** — bot responds with text prompt asking for input; format accepted: `DD.MM.YYYY HH:MM` or `DD/MM/YYYY HH:MM`. Bot validates future date and responds with error message if invalid, re-prompting. Locale-aware prompt.
4. **Location** — bot responds with inline keyboard showing previously used locations + "Enter new location" option. If new, bot responds with text prompt expecting text input.
5. **Max participants** — bot responds with text prompt; expects integer 2–100. Responds with validation error if out of range.
6. **Min participants** — bot responds with text prompt; expects integer 1–(max_participants). Must be ≤ max. Responds with validation error if invalid.
7. **Visibility** — bot responds with inline keyboard: Public | Private.
8. **Confirmation** — bot responds with a summary card (text message) showing all fields, plus inline keyboard: Confirm | Cancel.

On Confirm, game moves to `active` state and the bot responds with a success confirmation message (including the event card). On Cancel, the bot responds with a cancellation acknowledgement and clears FSM state.
...
- **Going** — adds to active participant list (if < max_participants) or reserve list. Bot responds with `answerCallbackQuery` ("Vote recorded") and updates the event message in-place via `editMessageText` / `editMessageReplyMarkup` to reflect updated counts.
- **Maybe** — recorded but does not count toward limits. Bot responds with `answerCallbackQuery` ("Vote recorded") and updates the event message in-place.
- **Not Going** — removes from participant list / reserve (if previously voted Going/Maybe). Bot responds with `answerCallbackQuery` ("Vote recorded") and updates the event message in-place. *(Scope addition beyond original requirements: original spec only defined "Going" and "Maybe". "Not Going" is included as essential UX — without it there is no way to withdraw a Going vote. This is treated as an implicit requirement.)*
...
- Graceful error handling: unrecognized commands return a friendly fallback message. Every webhook update must produce at least one response back to Telegram; the bot must never silently drop a user interaction.
