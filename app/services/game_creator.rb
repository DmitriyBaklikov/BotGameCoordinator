class GameCreator
  ...
  class << self
    ...
    def advance_to_datetime(user, controller, state, value, locale)
      ...
      controller.send_message(user.telegram_id, I18n.t("bot.enter_datetime", locale: locale))
    end
    ...
    def advance_to_confirmation(user, controller, state, value, locale)
      ...
      summary = build_summary(data, locale)
      controller.send_message(
        user.telegram_id,
    ...
    def handle_datetime_input(user, controller, state, text, locale)
      scheduled_at = parse_datetime(text)
      unless scheduled_at
        controller.send_message(user.telegram_id, I18n.t("bot.invalid_datetime_format", locale: locale))
        return
      end

      unless scheduled_at > Time.current
        controller.send_message(user.telegram_id, I18n.t("bot.datetime_must_be_future", locale: locale))
        return
      end
    ...
    def parse_datetime(text)
      DATETIME_FORMATS.each do |fmt|
        return Time.strptime(text.strip, fmt).in_time_zone
